...
export const popularLeagueExclusionTerms = [
  ...
  'women\'s', "women's", 'uefa nations league - women', 'uefa nations league women',
  ...
  'copa espírito santo', 'espirito santo', 'paulista série b',
...
export function shouldExcludeFromPopularLeagues(
  ...
  const isMajorInternationalCompetition = 
    ...
    (league.includes('qualifying') && (league.includes('world cup') || league.includes('euro')) && !league.includes('women')) ||
    (league.includes('international') && (league.includes('cup') || league.includes('championship')) && !league.includes('women')) ||
  ...
  return popularLeagueExclusionTerms.some(term => 
 ...
 */
export function isPopularLeagueSuitable(leagueName: string, country?: string): boolean {
  const league = safeSubstring(leagueName, 0).toLowerCase();
