 ...
 */
export function getTeamLogoSources(
  team: TeamData,
 ...
 */
export function createTeamLogoErrorHandler(
  team: TeamData,
  isNationalTeam = false,
  sport: string = "football",
) {
...
export function isNationalTeam(
  ...
  if (
    nationalTeamPatterns.some(
      (pattern) =>
        teamName === pattern ||
        teamName.startsWith(pattern + " ") ||
        teamName.endsWith(" " + pattern),
    )
  ) {
  ...
  const youthTeamMatch = teamName.match(
    /^(.+?)\s+(u|under)[-\s]?(17|19|20|21|23)$/i,
  );
  if (youthTeamMatch) {
    const baseCountry = youthTeamMatch[1].trim();
    console.log(
      `🔍 [isNationalTeam] Youth team detected: "${teamName}" -> base country: "${baseCountry}"`,
    );

    // Check if base country matches any national team pattern
    const isBaseCountryNational = nationalTeamPatterns.some(
      (pattern) =>
        baseCountry === pattern ||
        baseCountry.includes(pattern) ||
        pattern.includes(baseCountry),
    );

    if (isBaseCountryNational) {
      console.log(
        `✅ [isNationalTeam] Youth team "${teamName}" confirmed as national team`,
      );
      return true;
    }
  }
  ...
  if (specificNationalTeams.includes(teamName)) {
    console.log(
      `✅ [isNationalTeam] Specific national team detected: "${teamName}"`,
    );
    return true;
  }
  ...
  if (leagueName) {
    ...
    const isNationalCompetition = nationalCompetitionIndicators.some(
      (indicator) => leagueName.includes(indicator),
    );

    if (isNationalCompetition) {
      console.log(
        `🏆 [isNationalTeam] National competition context: "${leagueName}" for team "${teamName}"`,
      );

      // Special handling for UEFA Under-21 Championship and similar youth tournaments
      if (
        leagueName.includes("uefa under-21") ||
        leagueName.includes("under-21 championship")
      ) {
        console.log(
          `🇪🇺 [isNationalTeam] UEFA Under-21 Championship detected for: "${teamName}"`,
        );
      ...
      if (!hasClubIndicators) {
        console.log(
          `✅ [isNationalTeam] Team in national competition without club indicators: "${teamName}"`,
        );
        return true;
      }
    }
  ...
  console.log(
    `❌ [isNationalTeam] No national team indicators found for: "${teamName}" in league "${leagueName}"`,
  );
  return false;
}
