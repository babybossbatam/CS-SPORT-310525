...
// Global update manager to handle selective updates efficiently
class SelectiveMatchUpdater {
  private subscribers = new Map<number, Set<(data: MatchState) => void>>();
  private matchStates = new Map<number, MatchState>();
  private updateIntervals = new Map<number, NodeJS.Timeout>();

  subscribe(matchId: number, callback: (data: MatchState) => void): () => void {
  ...
  private startPolling(matchId: number) {
    // Poll every 15 seconds for live match updates
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (fixture) {
            ...
            const previousState = this.matchStates.get(matchId);
            const hasChanged = !previousState || 
              previousState.goals.home !== newState.goals.home ||
              previousState.goals.away !== newState.goals.away ||
              previousState.status.short !== newState.status.short ||
              previousState.status.elapsed !== newState.status.elapsed;

            if (hasChanged) {
              this.matchStates.set(matchId, newState);
              this.notifySubscribers(matchId, newState);
            }

            // Stop polling if match is finished
            const finishedStatuses = ["FT", "AET", "PEN", "AWD", "WO", "ABD", "CANC", "SUSP"];
            if (finishedStatuses.includes(newState.status.short)) {
              this.stopPolling(matchId);
            }
          }
        }
      } catch (error) {
        console.warn(`⚠️ [SelectiveUpdate] Failed to update match ${matchId}:`, error);
      }
    }, 15000);
  ...
  private stopPolling(matchId: number) {
  ...
  private notifySubscribers(matchId: number, data: MatchState) {
    ...
    if (subscribers) {
      subscribers.forEach(callback => {
        try {
          ...
          console.warn(`⚠️ [SelectiveUpdate] Subscriber callback error:`, error);
        }
      });
    }
  }

  getState(matchId: number): MatchState | null {
    return this.matchStates.get(matchId) || null;
  }
}

const selectiveMatchUpdater = new SelectiveMatchUpdater();

/**
 * Hook to use selective match updates
 * Only triggers re-renders when match data actually changes
 */
export const useSelectiveMatchUpdate = (
  matchId: number,
  initialMatch: any,
  enableUpdates: boolean = true
): MatchState | null => {
  ...
  useEffect(() => {
    if (!enableUpdates || !matchId) return;
    ...
    const unsubscribe = selectiveMatchUpdater.subscribe(matchId, (updatedData) => {
      ...
      if (!isMountedRef.current) {
        console.log(`🚫 [SelectiveUpdate] Skipping update for unmounted match ${matchId}`);
        return;
      }

      console.log(`🔄 [SelectiveUpdate] Match ${matchId} updated:`, {
        goals: `${updatedData.goals.home}-${updatedData.goals.away}`,
        status: updatedData.status.short,
        elapsed: updatedData.status.elapsed
      });

      setMatchState(updatedData);
    });

    // Cleanup subscription on unmount or dependency change
    return unsubscribe;
  }, [matchId, enableUpdates, initialMatch.teams?.home?.name, initialMatch.teams?.away?.name]);

  // Return current state for live matches, null for non-live matches
  return enableUpdates ? matchState : null;
};
