...
const translations = {
  en: {
    ...
    football_leagues: "FootballLeagues",
    ...
    draw: "Draw",
    data_from: "Data from",
    rapidapi_predictions: "RapidAPI Predictions",
    team_statistics: "Team Statistics",
    prediction_data_not_available: "Prediction data not available for this match",
    no_fixture_id_provided: "No fixture ID provided",
  ...
  "en-us": {
    ...
    no_live_matches_main:
      "It doesn't happen often, but there are no live games being played right now.",
    check_later: "Check it out later",
  ...
  es: {
    ...
    world: "Mundo",
    ended: "Finalizado",
    just_finished: "Recién Terminado",
    upcoming: "Próximo",
    ...
    no_live_matches_main:
      "No ocurre a menudo, pero no hay partidos en vivo en este momento.",
    ...
    stats: "Estadísticas",
    trends: "Tendencias",
    head_to_head: "Historial de Enfrentamientos",
    h2h: "H2H",
    groups: "Grupos",
    ...
    draw: "Empate",
    data_from: "Datos de",
    rapidapi_predictions: "Predicciones RapidAPI",
    team_statistics: "Estadísticas del Equipo",
    prediction_data_not_available: "Datos de predicción no disponibles para este partido",
    no_fixture_id_provided: "No se proporcionó ID del partido",
  ...
  "zh-hk": {
    ...
    draw: "和局",
    data_from: "數據來源",
    rapidapi_predictions: "RapidAPI 預測",
    team_statistics: "球隊統計",
    prediction_data_not_available: "此比賽沒有預測數據",
    no_fixture_id_provided: "未提供比賽ID",
  ...
  zh: {
    ...
    draw: "和局",
    data_from: "数据来源",
    rapidapi_predictions: "RapidAPI 预测",
    team_statistics: "球队统计",
    prediction_data_not_available: "此比赛没有预测数据",
    no_fixture_id_provided: "未提供比赛ID",
  ...
  de: {
    ...
    world: "Welt",
    ended: "Beendet",
    just_finished: "Gerade beendet",
    upcoming: "Bevorstehend",
    no_matches_available: "Keine Spiele verfügbar",
    no_live_matches_main:
      "Es passiert nicht oft, aber im Moment finden keine Live-Spiele statt.",
    ...
    probable_lineups: "Voraussichtliche Aufstellungen",
    stats: "Statistiken",
    trends: "Trends",
    ...
    draw: "Unentschieden",
    data_from: "Daten von",
    rapidapi_predictions: "RapidAPI Vorhersagen",
    team_statistics: "Teamstatistiken",
    prediction_data_not_available: "Vorhersagedaten für dieses Spiel nicht verfügbar",
    no_fixture_id_provided: "Keine Spiel-ID angegeben",
  ...
  it: {
    ...
    lineups: "Formazioni",
    probable_lineups: "Formazioni Probabili",
    stats: "Statistiche",
    trends: "Tendenze",
    head_to_head: "Confronto Diretto",
    h2h: "H2H",
    groups: "Gruppi",
    predictions: "Previsioni",
    who_will_win: "Chi vincerà?",
    whoWillWin: "Chi vincerà?",
    totalVotes: "Voti Totali",
    draw: "Pareggio",
    data_from: "Dati da",
    rapidapi_predictions: "Previsioni RapidAPI",
    team_statistics: "Statistiche Squadra",
    prediction_data_not_available: "Dati di previsione non disponibili per questa partita",
    no_fixture_id_provided: "Nessun ID partita fornito",
  ...
  pt: {
    ...
    lineups: "Escalações",
    probable_lineups: "Escalações Prováveis",
    stats: "Estatísticas",
    trends: "Tendências",
    ...
    prediction_data_not_available: "Dados de previsão não disponíveis para este jogo",
    no_fixture_id_provided: "Nenhum ID de jogo fornecido",
...
export const LanguageProvider: React.FC<{
  ...
  useEffect(() => {
    ...
    const initializeTranslations = async () => {
      try {
        console.log(
          `🤖 [LanguageContext] Initializing smart team translation for language: ${currentLanguage}`,
        );
        ...
        console.log(
          `✅ [LanguageContext] Smart team translation initialized successfully for ${currentLanguage}:`,
          stats,
        );
        ...
        testTeams.forEach((team) => {
          ...
          console.log(
            `🧪 [LanguageContext] Test translation: "${team}" -> "${translated}"`,
          );
        });
      } catch (error) {
        console.error(
          `❌ [LanguageContext] Failed to initialize smart team translation for ${currentLanguage}:`,
          error,
        );
  ...
  const translateLeagueName = (leagueName: string): string => {
    ...
    console.log(
      `🏆 [LanguageContext] Smart league translation: "${leagueName}" -> "${smartTranslation}"`,
    );

    if (smartTranslation !== leagueName) {
      console.log(
        `✅ [LanguageContext] Using smart league translation: "${smartTranslation}"`,
      );
      return smartTranslation;
    }

    // Fallback to team translation system for league names
    const teamSmartTranslation = smartTeamTranslation.translateLeagueName(
      leagueName,
      currentLanguage,
    );
    if (teamSmartTranslation !== leagueName) {
      console.log(
        `✅ [LanguageContext] Using team system league translation: "${teamSmartTranslation}"`,
      );
    ...
    for (const [pattern, translations] of Object.entries(
    ...
    for (const [chinesePattern, translations] of Object.entries(
      chineseLeaguePatterns,
    )) {
      if (leagueName.includes(chinesePattern)) {
        ...
        if (translation) {
          ...
          if (
            chinesePattern === "友誼賽" &&
            leagueName.toLowerCase().includes("clubs")
          ) {
            result =
              translations[currentLanguage as keyof typeof translations] ===
              "Friendlies"
    ...
    if (
      leagueName.toLowerCase().includes("liga profesional") &&
      (leagueName.toLowerCase().includes("argentina") ||
        leagueName.includes("阿根廷"))
    ) {
      return (
        {
          en: "Liga Profesional Argentina",
    ...
    if (
      leagueName.toLowerCase().includes("liga profesional argentina") ||
  ...
  const translateCountryName = (countryName: string): string => {
    ...
    console.log(
      `🌍 [LanguageContext] Translating country: "${countryName}" for language: ${currentLanguage}`,
    );
    ...
    console.log(
      `🤖 [LanguageContext] Smart country translation: "${countryName}" -> "${smartTranslation}"`,
    );

    if (smartTranslation !== countryName) {
      console.log(
        `✅ [LanguageContext] Using smart country translation: "${smartTranslation}"`,
      );
      return smartTranslation;
    }

    // Fallback to team translation system for country names
    const teamSmartTranslation = smartTeamTranslation.translateCountryName(
      countryName,
      currentLanguage,
    );
    if (teamSmartTranslation !== countryName) {
      console.log(
        `✅ [LanguageContext] Using team system country translation: "${teamSmartTranslation}"`,
      );
      return teamSmartTranslation;
    }
  ...
  const translateTeamName = (teamName: string): string => {
    ...
    console.log(
      `🌐 [LanguageContext] Translating team: "${teamName}" for language: ${currentLanguage}`,
    );

    // First try the smart translation system
    const smartTranslation = smartTeamTranslation.translateTeamName(
      teamName,
      currentLanguage,
    );
    console.log(
      `🤖 [LanguageContext] Smart translation result: "${teamName}" -> "${smartTranslation}"`,
    );

    if (smartTranslation !== teamName) {
      console.log(
        `✅ [LanguageContext] Using smart translation: "${smartTranslation}"`,
      );
      return smartTranslation;
    }
    ...
    if (
      normalizedTeamName.startsWith("fc ") ||
      normalizedTeamName.startsWith("cf ")
    ) {
    ...
    for (const suffix of suffixPatterns) {
      if (normalizedTeamName.endsWith(` ${suffix.toLowerCase()}`)) {
        const nameWithoutSuffix = teamName
          .substring(0, teamName.length - suffix.length - 1)
          .trim();
        if (teamTranslations[nameWithoutSuffix]) {
          return (
            teamTranslations[nameWithoutSuffix][currentLanguage] || teamName
          );
        }
      }
    }
  ...
  const translatePlayerName = useCallback(
    (playerName: string): string => {
      if (!playerName) return playerName;
      const translated = smartPlayerTranslation.translatePlayerName(
        playerName,
        currentLanguage,
      );
  ...
  const translatePositionName = useCallback(
    (position: string): string => {
      if (!position) return position;
      const translated = smartPlayerTranslation.translatePositionName(
        position,
        currentLanguage,
      );
      // Ensure we always return a string, not an object
  ...
  const getMatchStatusTranslation = (
    status: string,
    language: string,
  ): string => {
  ...
  const learnFromFixtures = (fixtures: any[]) => {
    try {
      ...
      console.log(
        `📚 [LanguageContext] Learning from ${fixtures.length} fixtures`,
      );
    } catch (error) {
  ...
  const learnFromPlayerData = (players: any[]) => {
    try {
      ...
      console.log(
        `🎯 [LanguageContext] Learning from ${players.length} players`,
      );
    } catch (error) {
