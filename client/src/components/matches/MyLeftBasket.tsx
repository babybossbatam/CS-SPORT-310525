...
const MyLeftBasket: React.FC<MyLeftBasketProps> = ({
  ...
  const filteredFixtures = useMemo(() => {
    ...
    console.log(
      `🔍 [MyLeftBasket UTC] Processing ${fixtures.length} fixtures for date: ${selectedDate}`,
    );
    ...
    const filtered = fixtures.filter((fixture) => {
      if (fixture.fixture.date && fixture.fixture.status?.short) {
        ...
        if (!shouldInclude) {
          console.log(
            `❌ [MyLeftBasket UTC FILTER] Match excluded: ${fixture.teams?.home?.name} vs ${fixture.teams?.away?.name}`,
            {
        ...
        console.log(
          `✅ [MyLeftBasket UTC FILTER] Match included: ${fixture.teams?.home?.name} vs ${fixture.teams?.away?.name}`,
          {
            fixtureUTCDate: fixture.fixture.date,
            extractedUTCDate: fixtureDateString,
            selectedDate,
    ...
    console.log(
      `✅ [MyLeftBasket UTC] After UTC filtering: ${filtered.length} matches for ${selectedDate}`,
    );
    return filtered;
  ...
  const handleMatchClick = (matchId: number) => {
    console.log('🏀 [MyLeftBasket] Basketball match clicked:', matchId);
    onMatchClick?.(matchId);
  };

  const handleMatchCardClick = (fixture: any) => {
    console.log('🏀 [MyLeftBasket] Basketball match card clicked:', {
