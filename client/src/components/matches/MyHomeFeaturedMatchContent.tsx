...
interface MyHomeFeaturedMatchContentProps {
  maxMatches?: number;
}
...
const MyHomeFeaturedMatchContent: React.FC<MyHomeFeaturedMatchContentProps> = ({
  maxMatches = 15,
}) => {
  ...
  const fetchFeaturedMatches = useCallback(
    async (forceRefresh = false) => {
      try {
        console.log('🔄 [MyHomeFeaturedMatchContent] Starting to fetch featured matches');
        ...
        console.log(
          "🔍 [MyHomeFeaturedMatchContent] Starting fetch with priority leagues:",
          priorityLeagueIds,
        );
        console.log(
          "🔍 [MyHomeFeaturedMatchContent] All featured league IDs:",
        ...
        try {
          if (forceRefresh) {
            console.log(
              "🔴 [MyHomeFeaturedMatchContent] Fetching live matches from dedicated endpoint",
            );
            ...
            if (Array.isArray(liveData)) {
              console.log(
                "🔍 [MyHomeFeaturedMatchContent] Processing live fixtures:",
                liveData.length,
              );
              ...
              console.log(
                "🔍 [MyHomeFeaturedMatchContent] Featured live fixtures:",
              ...
              liveFixtures = featuredLiveFixtures
                .filter((fixture: any) => {
                  ...
                  if (!isValid) {
                    console.log(
                      "❌ [MyHomeFeaturedMatchContent] Filtered out invalid fixture:",
                      {
                    ...
                    console.log(
                      "✅ [MyHomeFeaturedMatchContent] Valid featured live fixture:",
                      {
                        home: fixture.teams?.home?.name,
                        away: fixture.teams?.away?.name,
                        league: fixture.league?.name,
            ...
            console.log(
              `✅ [MyHomeFeaturedMatchContent] Found ${liveFixtures.length} live matches`,
            );
          }
        } catch (error) {
          console.error(
            "❌ [MyHomeFeaturedMatchContent] Error fetching live matches:",
            error,
          );
        }
        ...
        if (forceRefresh || allFixtures.length === 0) {
          ...
          for (const leagueId of priorityLeagueIds) {
            try {
              console.log(
                `🔍 [MyHomeFeaturedMatchContent] Fetching cached data for league ${leagueId}`,
              );
              ...
              if (Array.isArray(fixturesData)) {
                const cachedFixtures = fixturesData
                  .filter((fixture: any) => {
                    ...
                    if (shouldInclude) {
                      console.log(
                        `✅ [MyHomeFeaturedMatchContent] Including priority league ${leagueId} fixture:`,
                        {
                          home: fixture.teams?.home?.name,
                          away: fixture.teams?.away?.name,
                          league: fixture.league?.name,
          ...
          for (const dateInfo of dates) {
            try {
              console.log(
                `🔍 [MyHomeFeaturedMatchContent] Fetching cached data for ${dateInfo.label}: ${dateInfo.date}`,
              );
              ...
              console.error(
                `❌ [MyHomeFeaturedMatchContent] Error fetching cached data for ${dateInfo.label}:`,
                error,
              );
            }
          }
          ...
          if (allFixtures.length < 3) {
            console.log(
              `🔄 [MyHomeFeaturedMatchContent] Only ${allFixtures.length} fixtures found, expanding to all popular leagues`,
            );
            ...
            for (const dateInfo of dates) {
              try {
                ...
                console.error(
                  `❌ [MyHomeFeaturedMatchContent] Error in expanded search for ${dateInfo.label}:`,
                  error,
                );
              }
            }
        ...
        console.log(
          `📋 [MyHomeFeaturedMatchContent] Total unique fixtures found:`,
        ...
        for (const dateInfo of dates) {
          ...
          console.log(
            `✅ [MyHomeFeaturedMatchContent] Found ${fixturesForDay.length} featured matches for ${dateInfo.label}`,
          );
        ...
        console.error("❌ [MyHomeFeaturedMatchContent] Error:", error);
      } finally {
        setIsLoading(false);
      }
    },
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (hasLiveMatches) {
        console.log(
          "🔄 [MyHomeFeaturedMatchContent] Live matches detected, refreshing data",
        );
        fetchFeaturedMatches(false); // Background refresh without loading state
      } else {
        console.log(
          "⏸️ [MyHomeFeaturedMatchContent] No live matches, skipping refresh",
        );
      }
        ...
        <CardContent className="pt-0">
          <div className="space-y-4">
            ...
            <div className="text-center">
              <div className="h-4 w-20 mx-auto mb-2 bg-gray-200 animate-pulse" />
              <div className="h-8 w-16 mx-auto bg-gray-200 animate-pulse" />
            </div>

            {/* Teams display loading */}
            <div className="relative mt-4">
              <div className="flex relative h-[53px] rounded-md mb-8">
                <div className="w-full h-full flex justify-between relative">
                  <div className="flex items-center w-[45%]">
                    <div className="h-16 w-16 rounded-full bg-gray-200 animate-pulse" />
                    <div className="h-6 w-24 ml-4 bg-gray-200 animate-pulse" />
                  </div>
                  ...
                  <div className="flex items-center justify-end w-[45%]">
                    <div className="h-6 w-24 mr-4 bg-gray-200 animate-pulse" />
                    <div className="h-16 w-16 rounded-full bg-gray-200 animate-pulse" />
                  </div>
                </div>
              </div>
              ...
              <div className="text-center">
                <div className="h-4 w-64 mx-auto bg-gray-200 animate-pulse" />
              </div>
            </div>
            ...
            <div className="flex justify-around border-t border-gray-200 pt-4">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="flex flex-col items-center">
                  <div className="h-5 w-5 mb-1 bg-gray-200 animate-pulse" />
                  <div className="h-3 w-16 bg-gray-200 animate-pulse" />
                </div>
              ))}
            </div>

            {/* Navigation indicators loading */}
...
export default MyHomeFeaturedMatchContent;
