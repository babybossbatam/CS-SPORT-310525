...
import '../../styles/MyLiveMatchTracker.css';
...
interface MyLiveMatchTrackerProps {
  matchId?: number;
  homeTeam?: any;
  awayTeam?: any;
  isLive?: boolean;
...
const MyLiveMatchTracker: React.FC<MyLiveMatchTrackerProps> = ({
  matchId,
  homeTeam,
  awayTeam,
  isLive = false,
  ...
  useEffect(() => {
    const fetchSportradarMatchId = async () => {
      if (matchId && isLive) {
        try {
          console.log('🔍 [MyLiveMatchTracker] Mapping RapidAPI match ID to Sportradar:', matchId);
          ...
          if (response.ok) {
            ...
            if (data.success && data.sportradarMatchId) {
              console.log('✅ [MyLiveMatchTracker] Mapped to Sportradar match ID:', data.sportradarMatchId);
              setSportradarMatchId(data.sportradarMatchId);
              return;
            }
          }
        } catch (error) {
          console.warn('⚠️ [MyLiveMatchTracker] Failed to map Sportradar match ID:', error);
        }
      }
      
      // Keep default fallback ID
      console.log('🔄 [MyLiveMatchTracker] Using default Sportradar match ID:', sportradarMatchId);
    };
  ...
  useEffect(() => {
    const loadSportradarWidget = () => {
      try {
        ...
        script.async = true;
        script.onload = () => {
          console.log('🎯 [MyLiveMatchTracker] Sportradar script loaded successfully');
          scriptLoadedRef.current = true;
          setTimeout(() => {
            initializeWidget();
          }, 1000);
        ...
        script.onerror = () => {
          console.error('🚫 [MyLiveMatchTracker] Failed to load Sportradar script');
        ...
        console.error('🚫 [MyLiveMatchTracker] Error loading Sportradar script:', error);
    ...
    const initializeWidget = () => {
      console.log('🔄 [MyLiveMatchTracker] Attempting widget initialization...', {
        hasSIR: !!window.SIR,
      ...
      if (window.SIR && widgetRef.current && !widgetInitializedRef.current) {
        try {
          console.log('🎯 [MyLiveMatchTracker] Initializing Sportradar widget with matchId:', sportradarMatchId);
          
          // Clear any existing content
          widgetRef.current.innerHTML = '';
          
          // Initialize widget with enhanced configuration
          window.SIR("addWidget", ".sr-widget-live-tracker", "match.lmtPlus", {
          ...
          setIsLoading(false);
          setError(null);
          console.log('✅ [MyLiveMatchTracker] Sportradar widget initialized successfully');
          
        } catch (error) {
          console.error('🚫 [MyLiveMatchTracker] Error initializing Sportradar widget:', error);
          setError('Failed to initialize widget: ' + (error as Error).message);
          setIsLoading(false);
        }
      } else if (!window.SIR && scriptLoadedRef.current) {
        console.error('🚫 [MyLiveMatchTracker] SIR not available after script loaded');
  ...
  return (
    <div className={`live-match-tracker-module-placeholder-container ${className}`}>
      <div className="bg-white rounded-lg overflow-hidden shadow-lg border border-gray-100">
        ...
        {/* Loading State */}
        {isLoading && (
          <div className="live-match-tracker-module-loading p-8">
        ...
        <div className="relative">
          <div 
            ref={widgetRef}
            className="sr-widget sr-widget-live-tracker w-full"
          ...
          {isLoading && (
            <div className="live-match-tracker-module-cover absolute inset-0 bg-white bg-opacity-90 flex items-center justify-center">
              <div className="text-center">
...
export default MyLiveMatchTracker;
