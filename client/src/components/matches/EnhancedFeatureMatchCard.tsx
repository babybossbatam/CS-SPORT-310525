...
import { format, addHours, subHours, parse, parseISO, differenceInMinutes, differenceInHours, formatDistance } from 'date-fns';
...
const EnhancedFeatureMatchCard = () => {
  ...
  const fetchMatchesForFeaturedLeagues = useCallback(async () => {
    ...
    try {
      ...
      // Filter matches based on timing criteria
      const now = new Date();
      
      // 1. Upcoming matches: Within 8 hours from now
      const upcomingMatches = allFixtures.filter(match => {
        ...
        return hoursDiff >= 0 && hoursDiff <= 8 && match.fixture.status.short !== 'FT';
      });
      ...
      // 3. Recently finished matches: Finished within last 8 hours
      const finishedMatches = allFixtures.filter(match => {
        if (match.fixture.status.short !== 'FT') return false;
        
        const matchDate = parseISO(match.fixture.date);
        const hoursSince = differenceInHours(now, matchDate);
        return hoursSince >= 0 && hoursSince <= 8;
      });
      
      // Prioritize: Live > Upcoming > Finished
      let matches: FixtureResponse[] = [];
      
      if (liveMatches.length > 0) {
        matches = liveMatches;
        setMatchType(MatchType.LIVE);
      } else if (upcomingMatches.length > 0) {
        // Sort upcoming matches by start time (closest first)
        matches = upcomingMatches.sort((a, b) => 
          new Date(a.fixture.date).getTime() - new Date(b.fixture.date).getTime()
        );
        setMatchType(MatchType.UPCOMING);
      } else if (finishedMatches.length > 0) {
        // Sort finished matches by end time (most recent first)
        matches = finishedMatches.sort((a, b) => 
          new Date(b.fixture.date).getTime() - new Date(a.fixture.date).getTime()
        );
        setMatchType(MatchType.FINISHED);
      }
