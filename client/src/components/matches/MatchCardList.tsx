...
const MatchCardList: React.FC<MatchCardListProps> = ({
  ...
  const { data: fixtures = [], isLoading } = useCachedQuery(
    ["match-cards-fixtures", selectedDate],
    async () => {
      console.log(`🔄 [MatchCardList] Fetching data for date: ${selectedDate}`);
      const response = await apiRequest("GET", `/api/fixtures/date/${selectedDate}?all=true`);
      const data = await response.json();
      console.log(`✅ [MatchCardList] Received ${data?.length || 0} fixtures for ${selectedDate}`);
      return data;
    },
    {
  ...
  const processedMatches = useMemo(() => {
    ...
    console.log(`🔍 [MatchCardList] Processing ${fixtures.length} fixtures for date: ${selectedDate}`);
    ...
    const filtered = fixtures.filter((fixture) => {
      ...
      if (isRestrictedUSLeague(leagueId, fixture.league.country)) {
        return false;
      }
