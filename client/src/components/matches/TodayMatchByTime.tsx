...
interface TodayMatchByTimeProps {
  ...
  fixtures?: FixtureData[]; // Accept fixtures from parent component
}
...
const TodayMatchByTime: React.FC<TodayMatchByTimeProps> = ({
  ...
  liveFilterActive,
  onMatchCardClick,
  fixtures = [], // Default to empty array if no fixtures provided
}) => {
  ...
  // Use provided fixtures instead of fetching separately
  const allFixtures = fixtures;
  const isLoading = false;
  const error = null;

  console.log(
    `🎯 [TodayMatchByTime] Using ${allFixtures.length} fixtures passed from parent component for ${selectedDate}`,
  );
