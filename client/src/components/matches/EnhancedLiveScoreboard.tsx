...
export function EnhancedLiveScoreboard({
  ...
  const [selectedLeague, setSelectedLeague] = useState<string>("Serie A");
  ...
  useEffect(() => {
    ...
    if (selectedLeague !== "all") {
      if (selectedLeague === "europe") {
        // European leagues filter
        const europeanLeagueIds = [39, 140, 135, 78];
        filtered = sortedMatches.filter((match) =>
          europeanLeagueIds.includes(match.league.id),
        );
      } else {
        filtered = sortedMatches.filter(
  ...
  const sortedByDate = [...filteredMatches].sort(
    (a, b) =>
      new Date(a.fixture.date).getTime() - new Date(b.fixture.date).getTime(),
  );
