...
const MyScoresLeagueComponent: React.FC<MyScoresLeagueProps> = ({
  ...
  const getCachedEndedMatches = useCallback((date: string, leagueId: number): FixtureData[] => {
    try {
      ...
      if (cachedDate !== date) {
        console.log(`🚨 [MyScoresLeague] Date mismatch in cache - cached: ${cachedDate}, requested: ${date}, clearing cache`);
      ...
      if (cacheAge < maxCacheAge) {
        ...
        if (validFixtures.length !== fixtures.length) {
          console.log(`🚨 [MyScoresLeague] Found ${fixtures.length - validFixtures.length} fixtures with wrong dates in cache, clearing`);
          localStorage.removeItem(cacheKey);
          return [];
        }

        console.log(`✅ [MyScoresLeague] Using cached ended matches for league ${leagueId} on ${date}: ${validFixtures.length} matches`);
        return validFixtures;
      } else {
        // Remove expired cache
        localStorage.removeItem(cacheKey);
        console.log(`⏰ [MyScoresLeague] Removed expired cache for league ${leagueId} on ${date} (age: ${Math.round(cacheAge / 60000)}min)`);
      }
    } catch (error) {
  ...
  const cacheEndedMatches = useCallback((date: string, leagueId: number, fixtures: FixtureData[]) => {
    try {
      ...
      console.log(`💾 [MyScoresLeague] Cached ${endedFixtures.length} ended matches for league ${leagueId} on ${date}`);
    } catch (error) {
  ...
  const updateLiveMatchData = useCallback(async () => {
    try {
      ...
      if (currentLiveMatches.length === 0) {
        console.log('🚫 [MyScoresLeague] No live matches found, skipping live data update');
        return;
      }

      console.log(`🔴 [MyScoresLeague] Updating ${currentLiveMatches.length} live matches`);
  ...
  const fetchLeagueData = useCallback(async (isUpdate = false) => {
    ...
    try {
      ...
      if (!isUpdate) {
        try {
          console.log(`🔴 [MyScoresLeague] Fetching live fixtures for initial load`);
          ...
          if (Array.isArray(liveData)) {
            ...
            if (relevantLiveFixtures.length > 0) {
              console.log(`🔴 [MyScoresLeague] Found ${relevantLiveFixtures.length} live fixtures from target leagues`);
            }

            allFixtures.push(...relevantLiveFixtures);
          }
        } catch (liveError) {
          console.warn("🔴 [MyScoresLeague] Failed to fetch live fixtures:", liveError);
        }
      }
      ...
      for (const leagueId of leagueIds) {
        try {
          ...
          console.log(`MyScoresLeague - League ${leagueId} fixtures count:`, freshFixtures?.length || 0);
          ...
          if (Array.isArray(freshFixtures)) {
            ...
            console.log(`🎯 [MyScoresLeague] League ${leagueId}: ${freshFixtures.length} → ${filteredFixtures.length} fixtures after date filtering`);

            // Log sample fixtures for debugging
            if (filteredFixtures.length > 0) {
              filteredFixtures.slice(0, 3).forEach(fixture => {
                console.log(`MyScoresLeague - Fixture ${fixture.fixture.id}:`, {
      ...
      if (!isUpdate && leagueIds.length > 0) {
        try {
          ...
          console.log(`MyScoresLeague - Primary league ${leagueIds[0]} info:`, leagueData);
      ...
      console.log(`📊 [MyScoresLeague] Final result: ${allFixtures.length} fixtures`);

      // Log breakdown by league
      leagueIds.forEach(leagueId => {
        const leagueFixtures = allFixtures.filter(f => f.league.id === leagueId);
        if (leagueFixtures.length > 0) {
          console.log(`🎯 [MyScoresLeague] League ${leagueId}: ${leagueFixtures.length} fixtures`);
        }
      });
  ...
  useEffect(() => {
    const fetchAllLeagueFixtures = async () => {
      ...
      try {
        ...
        console.log(`🎯 [MyScoresLeague] Smart fetching for date: ${selectedDate}`);
        ...
        console.log(`📊 [MyScoresLeague] Got ${allDateFixtures.length} total fixtures for ${selectedDate}`);
        ...
        const promises = leagueIds.map(async (leagueId) => {
          ...
          console.log(`✅ [MyScoresLeague] League ${leagueId}: Found ${fixtures.length} fixtures for ${selectedDate}`);
        ...
        console.error('❌ [MyScoresLeague] Error fetching league fixtures:', error);
    ...
    if (cached && !liveFilterActive) {
      try {
        ...
        if (age < maxAge) {
          console.log(`💾 [MyScoresLeague] Using cached data for ${selectedDate} (age: ${Math.round(age/1000)}s)`);
  ...
  useEffect(() => {
    const cleanupOldCache = () => {
      try {
          ...
          try {
            ...
            if (cached) {
              ...
              if (cacheAge > maxAge) {
                ...
                cleanedCount++;
                console.log(`🗑️ [MyScoresLeague] Removed stale cache: ${key} (age: ${Math.round(cacheAge / 60000)}min)`);
              }
            }
          } catch (error) {
        ...
        if (cleanedCount > 0) {
          console.log(`🧹 [MyScoresLeague] Cleaned up ${cleanedCount} old cache entries`);
        }
      } catch (error) {
  ...
  useEffect(() => {
    const clearDateSpecificCache = () => {
      try {
          ...
          try {
            ...
            if (cached) {
              ...
              if (cacheAge < 2 * 60 * 60 * 1000 && cachedDate !== selectedDate) {
                localStorage.removeItem(key);
                console.log(`🗑️ [MyScoresLeague] Cleared cross-date cache: ${key} (cached: ${cachedDate}, selected: ${selectedDate})`);
              }
            }
          } catch (error) {
            localStorage.removeItem(key);
        ...
        console.log(`🔄 [MyScoresLeague] Cache cleared for date change to ${selectedDate}`);
      } catch (error) {
  ...
  useEffect(() => {
    ...
    console.log(`🎯 [MyScoresLeague] Data fetched for ${selectedDate}, selective updates will handle live matches`);
  ...
  console.log("🏆 [MyScoresLeague FRIENDLIES] Total Friendlies fixtures:", friendliesFixtures.length);
  console.log("🇮🇶 [MyScoresLeague IRAQI] Total Iraqi League fixtures:", iraqiFixtures.length);
  console.log("🇦🇷 [MyScoresLeague COPA ARG] Total Copa Argentina fixtures:", copaArgentinaFixtures.length);
  ...
  console.log(`🏆 [MyScoresLeague FRIENDLIES] After date filtering: ${friendliesFiltered.length} matches for ${selectedDate}`);
  console.log(`🇮🇶 [MyScoresLeague IRAQI] After date filtering: ${iraqiFiltered.length} matches for ${selectedDate}`);
  console.log(`🇦🇷 [MyScoresLeague COPA ARG] After date filtering: ${copaArgentinaFiltered.length} matches for ${selectedDate}`);
  ...
  useEffect(() => {
    ...
    fixtures.forEach((fixture) => {
      ...
      if (isLiveOrUpcoming && previousStatus && previousStatus !== currentStatus) {
        console.log(`🔄 [MyScoresLeague] Status change: ${fixture.teams?.home?.name} vs ${fixture.teams?.away?.name} (${previousStatus} → ${currentStatus})`);
      ...
      if (previousScore && 
        ...
        console.log(`⚽ [MyScoresLeague] Goal scored: ${fixture.teams?.home?.name} vs ${fixture.teams?.away?.name}`);
