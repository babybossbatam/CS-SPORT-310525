...
const TodayPopularFootballLeaguesNew: React.FC<TodayPopularFootballLeaguesNewProps> = ({ 
  ...
  const fixturesByCountry = allFixtures.reduce((acc: any, fixture: any) => {
    ...
    if (!acc[country]) {
      acc[country] = {
        country,
        flag: getCountryFlag(country, league.flag),leagues: {},
  ...
  if (timeFilterActive) {
    ...
    const allMatches = sortedCountries.flatMap((countryData: any) =>
      Object.values(countryData.leagues).flatMap((leagueData: any) =>
    ...
    const sortedMatches = allMatches.sort((a: any, b: any) => {
      ...
      // PRIORITY 3: Upcoming matches second, sorted by time (nearest first)
      ...
      // PRIORITY 4: Finished matches last, sorted by time (most recent first)
              ...
              {sortedMatches
                ...
                .map((match: any) => (
                  <div 
                    ...
                    className="bg-white hover:bg-gray-50 transition-all duration-200 cursor-pointer border-b border-gray-100 last:border-b-0 group relative"
                  >
                    <div className="flex items-center px-3 py-2">
                      {/* Star icon with hover slide effect */}
                      <div className="absolute left-1 opacity-0 group-hover:opacity-100 transition-all duration-300 transform -translate-x-2 group-hover:translate-x-0">
                        <svg 
                ...
                <Card key={leagueData.league.id} className={`overflow-hidden ${leagueIndex === 0 ? '-mt-4' : ''}`}>
                  ...
                  <CardContent className="p-0">
                    ...
                    {leagueData.matches.map((match: any) => (
                      <div
                        key={match.fixture.id}
                        className="bg-white hover:bg-gray-50 transition-all duration-200 cursor-pointer border-b border-gray-300 last:border-b-0 group relative"
                      >
                        <div className="flex items-center px-3 py-2">
                          {/* Star icon with hover slide effect */}
                          <div className="absolute left-1 opacity-0 group-hover:opacity-100 transition-all duration-300 transform -translate-x-2 group-hover:translate-x-0">
