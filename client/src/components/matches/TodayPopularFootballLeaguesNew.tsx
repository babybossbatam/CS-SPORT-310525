...
> = ({
  ...
  // Consolidate all matches from all countries and leagues into a single array
  ...
  // Sort all matches by priority: Live > Upcoming > Recent Finished
  const sortedAllMatches = useMemo(() => {
    return allMatches.sort((a: any, b: any) => {
      ...
      if (aLive && bLive) {
        ...
        // If same elapsed time, sort by kickoff time
        return aTime - bTime;
      }

      // PRIORITY 2: Upcoming matches come second, sorted by time
      ...
      // PRIORITY 3: Recent finished matches (within 8 hours) come third
      const aRecentFinished = aFinished && (now - aTime) <= (8 * 60 * 60 * 1000);
      const bRecentFinished = bFinished && (now - bTime) <= (8 * 60 * 60 * 1000);

      if (aRecentFinished && !bRecentFinished) return -1;
      if (!aRecentFinished && bRecentFinished) return 1;

      if (aRecentFinished && bRecentFinished) {
        return bTime - aTime; // Most recent finished first
      }

      // PRIORITY 4: Other finished matches
    ...
    <>
      ...
      <Card className="border bg-card text-card-foreground shadow-md overflow-hidden mb-4">
        <CardContent className="p-0">
          <div className="space-y-0">
            {sortedAllMatches
              .slice(0, showTop20 ? 20 : undefined)
              .map((match: any) => (
                <LazyMatchItem key={match.fixture.id}>
                  <div className="match-card-container group">
                    ...
                    <div className="match-content-container">
                      {/* League info - small badge */}
                      <div className="absolute top-1 left-1 z-10">
                        <span className="text-xs bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded-sm">
                          {match.leagueInfo?.name && match.leagueInfo.name.length > 20 
                            ? match.leagueInfo.name.substring(0, 20) + "..."
                            : match.leagueInfo?.name || "Unknown"}
                        </span>
                      </div>
