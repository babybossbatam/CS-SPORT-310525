...
interface TodayBasketballCardProps {
  fixtures: any[];
  onMatchClick: (matchId: number) => void;
  onMatchCardClick?: (fixture: any) => void;
}

export const TodayBasketballCard = ({
  fixtures,
  onMatchClick,
  onMatchCardClick,
}: TodayBasketballCardProps) => {
  ...
  // Dedicated date display function for TodayBasketballCard
  const getTodayBasketballDisplayName = () => {
    const today = getCurrentUTCDateString();
    const yesterday = format(subDays(parseISO(today), 1), "yyyy-MM-dd");
    const tomorrow = format(addDays(parseISO(today), 1), "yyyy-MM-dd");

    if (selectedDate === today) {
      return "Today's Basketball";
    } else if (selectedDate === yesterday) {
      return "Yesterday's Basketball";
    } else if (selectedDate === tomorrow) {
      return "Tomorrow's Basketball";
    } else {
  ...
  // Fetch live basketball fixtures when live filter is active
  const { data: sharedLiveFixtures = [], isLoading: isLoadingLive } = useQuery({
    queryKey: ["shared-live-basketball-fixtures"],
    queryFn: async () => {
      console.log("Fetching shared live basketball fixtures");
      const response = await apiRequest("GET", "/api/basketball/fixtures/live");
      const data = await response.json();
      console.log(`Received ${data.length} shared live basketball fixtures`);
      return data;
    },
    staleTime: 30000,
    gcTime: 2 * 60 * 1000,
  ...
  console.log(`🏀 [TodayBasketballCard] Rendering for date: ${selectedDate}`);

  const handleMatchCardClick = (fixture: any) => {
    console.log('🏀 [TodayBasketballCard] Basketball match card clicked:', {
      ...
      source: 'TodayBasketballCard'
    });
    onMatchCardClick?.(fixture);
  };

  const handleLiveMatchClick = (fixture: any) => {
    console.log('🔴 [TodayBasketballCard] LIVE Basketball match card clicked from LiveMatchForAllCountry:', {
      fixtureId: fixture.fixture?.id,
      teams: `${fixture.teams?.home?.name} vs ${fixture.teams?.away?.name}`,
      league: fixture.league?.name,
      country: fixture.league?.country,
  ...
  return (
    <>
              ...
              >
                <Calendar
                  ...
                  classNames={{
                    ...
                    day_selected:
                      "bg-orange-500 text-white hover:bg-orange-300 hover:text-gray-900 focus:bg-orange-400 focus:text-white rounded-full",
                    day_today:
                      "text-orange-500 font-normal hover:text-gray-900 hover:bg-orange-300 rounded-full ",
                    day_outside:
                ...
                <div className="flex justify-center pt-2 ">
                  <button
                    onClick={goToToday}
                    className="text-orange-500 hover:bg-orange-300 text-sm font-medium"
                  >
                    Today
                  </button>
                </div>
          ...
          <button
            onClick={() => {
              if (!liveFilterActive) {
                ...
                if (!timeFilterActive) {
                  // Return to default view (MyBasketballPopularLeague)
                  setTimeFilterActive(false);
                } else {
      ...
      {liveFilterActive && timeFilterActive ? (
        ...
        // Time only - show TodayMatchByTime component
        <TodayMatchByTime
        ...
        // Neither filter active - show default basketball view
        <>
          <MyBasketballPopularLeague
            selectedDate={selectedDate}
            onMatchCardClick={handleMatchCardClick}
          />
...
export default TodayBasketballCard;
