...
const MyBasketballPopularLeague = ({
  ...
  console.log("🏀 [MyBasketballPopularLeague] Received match data:", {
    hasMatch: !!match,
  ...
  const {
    ...
    queryFn: async () => {
      console.log(
        `🏀 [MyBasketballPopularLeague] Fetching fixtures for ${basketballLeagueIds.length} basketball leagues on ${selectedDate}:`,
        basketballLeagueIds,
      );

      const promises = basketballLeagueIds.map(async (leagueId) => {
        try {
          const response = await fetch(`/api/basketball/leagues/${leagueId}/fixtures`);
          if (!response.ok) {
            console.log(
              `❌ [MyBasketballPopularLeague] Failed to fetch league ${leagueId}: ${response.status} ${response.statusText}`,
            );
          ...
          console.log(
            `✅ [MyBasketballPopularLeague] League ${leagueId}: ${fixtures.length} fixtures`,
          );
          return { leagueId, fixtures, error: null };
        } catch (error) {
          console.error(
            `❌ [MyBasketballPopularLeague] Error fetching league ${leagueId}:`,
            error,
          );
      ...
      console.log(`🔄 [MyBasketballPopularLeague] Fetch results:`, {
  ...
  const fixturesByLeague = useMemo(() => {
    console.log(
      `🔍 [MyBasketballPopularLeague] Processing fixtures for date ${selectedDate}:`,
      {
    ...
    if (!allFixtures?.length) {
      console.log(`❌ [MyBasketballPopularLeague] No fixtures available`);
      return {};
    }
    ...
    Object.values(grouped).forEach((group) => {
      group.fixtures.sort((a, b) => {
        ...
        const getStatusPriority = (status: string) => {
          ...
          if (
            ["LIVE", "Q1", "Q2", "HT", "Q3", "Q4", "OT", "BT"].includes(
              status,
            )
          ) {
            return 1;
    ...
    console.log(
      `✅ [MyBasketballPopularLeague] Date filtered fixtures for ${selectedDate}:`,
      {
  ...
  const handleMatchClick = (fixture: FixtureData) => {
    console.log("🏀 [MyBasketballPopularLeague] Basketball match card clicked:", {
      fixtureId: fixture.fixture?.id,
      teams: `${fixture.teams?.home?.name} vs ${fixture.teams?.away?.name}`,
      league: fixture.league?.name,
      status: fixture.fixture?.status?.short,
      source: "MyBasketballPopularLeague",
    });
    ...
    <>
      ...
      {leagueEntries
        ...
        .map(([leagueId, { league, fixtures }]) => {
          ...
          return (
            <Card
              key={`basketballleague-${leagueIdNum}`}
              className="border bg-card text-card-foreground shadow-md overflow-hidden league-card-spacing"
            >
                            ...
                            <div className="match-content-container">
                              ...
                              <div className="match-score-container">
                                {(() => {
                                  ...
                                  // For postponed matches and upcoming matches - show tip-off time
                                  if (
                                    status === "NS" ||
                                    status === "TBD" ||
                                    [
                                    ...
                                    // For postponed/cancelled matches, still show the tip-off time
                                    if (
                                      [
                                        "PST",
                                        "CANC",
                                    ...
                                    // If match is more than 2 hours overdue, show tip-off time but with postponed styling
                                    if (hoursAgo > 2) {
                                      const localTime =
                                        matchTime.toLocaleTimeString("en-US", {
                                          hour: "2-digit",
...
export default MyBasketballPopularLeague;