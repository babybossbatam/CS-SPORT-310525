...
const MyMainLayout_ES: React.FC<MyMainLayoutProps> = ({
  fixtures,
  loading = false,
  children,
}) => {
  ...
  const filteredFixtures = useMemo(() => {
    ...
    console.log(
      `🔍 [MyMainLayout_ES UTC] Procesando ${fixtures.length} partidos para la fecha: ${selectedDate}`,
    );
    ...
    const filtered = fixtures.filter((fixture) => {
      if (fixture.fixture.date && fixture.fixture.status?.short) {
        ...
        if (!shouldInclude) {
          console.log(
            `❌ [MyMainLayout_ES UTC FILTER] Partido excluido: ${fixture.teams?.home?.name} vs ${fixture.teams?.away?.name}`,
            {
              fixtureUTCDate: fixture.fixture.date,
              extractedUTCDate: fixtureDateString,
              selectedDate,
              status: fixture.fixture.status.short,
              reason: "desajuste de fecha UTC",
            },
          );
          return false;
        }
        ...
        console.log(
          `✅ [MyMainLayout_ES UTC FILTER] Partido incluido: ${fixture.teams?.home?.name} vs ${fixture.teams?.away?.name}`,
          {
            fixtureUTCDate: fixture.fixture.date,
            extractedUTCDate: fixtureDateString,
            selectedDate,
    ...
    if (processingTime > 50) {
      console.warn(
        `⚠️ [MyMainLayout_ES Performance] El filtrado tomó ${processingTime.toFixed(2)}ms`,
      );
    }

    console.log(
      `✅ [MyMainLayout_ES UTC] Después del filtrado UTC: ${filtered.length} partidos para ${selectedDate} (${processingTime.toFixed(2)}ms)`,
    );
    return filtered;
      ...
      >
        ...
        >
          ...
          {(!isMobile || !selectedFixture) && (
            ...
            >
              {/* Spanish Static Content */}
              <div className="text-center mb-4">
                <h1 className="text-2xl font-bold text-gray-800">Partidos de Fútbol de Hoy</h1>
                <p className="text-gray-600 mt-2">Resultados en vivo y calendario de todo el mundo</p>
              </div>
              ...
              {/* Spanish Footer Content */}
              <div className="text-center mt-6 text-sm text-gray-500">
                <p>Bienvenido a CS SPORT - ¡Tu destino definitivo para el Fútbol!</p>
                <p>Resultados en vivo de más de 1,000 competiciones en todo el mundo</p>
              </div>
            </div>
          )}
...
export default MyMainLayout_ES;
