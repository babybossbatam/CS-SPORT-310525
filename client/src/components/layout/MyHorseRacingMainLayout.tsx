...
const MyHorseRacingMainLayout: React.FC<MyHorseRacingMainLayoutProps> = ({
  ...
  console.log(`🌐 [MyHorseRacingMainLayout] Translation language: ${translationLanguage}`);

  // Simplified fixture filtering for horse racing
  const filteredFixtures = useMemo(() => {
    if (!fixtures?.length || !selectedDate || selectedDate === 'undefined') {
      console.warn('🚨 [MyHorseRacingMainLayout] Invalid data:', { fixturesLength: fixtures?.length, selectedDate });
      return [];
    }

    console.log(`🔍 [MyHorseRacingMainLayout] Processing ${fixtures.length} fixtures for date: ${selectedDate}`);
    ...
    console.log(`✅ [MyHorseRacingMainLayout] Filtered to ${filtered.length} matches for ${selectedDate}`);
