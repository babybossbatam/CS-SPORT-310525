...
const MyMainLayout_EN: React.FC<MyMainLayoutProps> = ({
  fixtures,
  loading = false,
  children,
}) => {
  ...
  const filteredFixtures = useMemo(() => {
    ...
    console.log(
      `🔍 [MyMainLayout_EN UTC] Processing ${fixtures.length} fixtures for date: ${selectedDate}`,
    );
    ...
    const filtered = fixtures.filter((fixture) => {
      if (fixture.fixture.date && fixture.fixture.status?.short) {
        ...
        if (!shouldInclude) {
          console.log(
            `❌ [MyMainLayout_EN UTC FILTER] Match excluded: ${fixture.teams?.home?.name} vs ${fixture.teams?.away?.name}`,
            {
        ...
        console.log(
          `✅ [MyMainLayout_EN UTC FILTER] Match included: ${fixture.teams?.home?.name} vs ${fixture.teams?.away?.name}`,
          {
            fixtureUTCDate: fixture.fixture.date,
            extractedUTCDate: fixtureDateString,
            selectedDate,
    ...
    if (processingTime > 50) {
      console.warn(
        `⚠️ [MyMainLayout_EN Performance] Filtering took ${processingTime.toFixed(2)}ms`,
      );
    }

    console.log(
      `✅ [MyMainLayout_EN UTC] After UTC filtering: ${filtered.length} matches for ${selectedDate} (${processingTime.toFixed(2)}ms)`,
    );
    return filtered;
          ...
          {(!isMobile || !selectedFixture) && (
            ...
            >
              ...
              <div className="text-center mb-4">
                <h1 className="text-2xl font-bold text-gray-800">Today's Football Matches</h1>
                <p className="text-gray-600 mt-2">Live scores and fixtures from around the world</p>
              </div>
              ...
              <div className="text-center mt-6 text-sm text-gray-500">
                <p>Welcome to CS SPORT - Your ultimate destination for Football!</p>
                <p>Live scores from over 1,000 competitions worldwide</p>
              </div>
            </div>
          )}
...
export default MyMainLayout_EN;
