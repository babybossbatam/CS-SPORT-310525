...
const MyMainLayout_ZH: React.FC<MyMainLayoutProps> = ({
  fixtures,
  loading = false,
  children,
}) => {
  ...
  const filteredFixtures = useMemo(() => {
    ...
    console.log(
      `🔍 [MyMainLayout_ZH UTC] 正在处理 ${fixtures.length} 场比赛，日期: ${selectedDate}`,
    );
    ...
    const filtered = fixtures.filter((fixture) => {
      if (fixture.fixture.date && fixture.fixture.status?.short) {
        ...
        if (!shouldInclude) {
          console.log(
            `❌ [MyMainLayout_ZH UTC FILTER] 比赛已排除: ${fixture.teams?.home?.name} vs ${fixture.teams?.away?.name}`,
            {
        ...
        console.log(
          `✅ [MyMainLayout_ZH UTC FILTER] 比赛已包含: ${fixture.teams?.home?.name} vs ${fixture.teams?.away?.name}`,
          {
            fixtureUTCDate: fixture.fixture.date,
            extractedUTCDate: fixtureDateString,
            selectedDate,
    ...
    if (processingTime > 50) {
      console.warn(
        `⚠️ [MyMainLayout_ZH Performance] 筛选耗时 ${processingTime.toFixed(2)}ms`,
      );
    }

    console.log(
      `✅ [MyMainLayout_ZH UTC] UTC筛选后: ${filtered.length} 场比赛，日期 ${selectedDate} (${processingTime.toFixed(2)}ms)`,
    );
    return filtered;
      ...
      >
        ...
        >
          ...
          {(!isMobile || !selectedFixture) && (
            ...
            >
              {/* Chinese Static Content */}
              <div className="text-center mb-4">
                <h1 className="text-2xl font-bold text-gray-800">今日足球比赛</h1>
                <p className="text-gray-600 mt-2">来自世界各地的即时比分和赛程</p>
              </div>
              ...
              <div className="text-center mt-6 text-sm text-gray-500">
                <p>欢迎来到 CS SPORT - 您的足球终极目的地！</p>
                <p>来自全球1000多个赛事的即时比分</p>
              </div>
            </div>
          )}
...
export default MyMainLayout_ZH;
