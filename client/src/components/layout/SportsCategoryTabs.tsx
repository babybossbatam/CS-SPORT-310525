...
const SportsCategoryTabs = () => {
  ...
  return (
    <Card className="rounded-none border-b shadow-lg fixed left-0 z-50 top-[77px] w-full bg-white/95 backdrop-blur-sm">
        <div className="flex justify-center items-center p-1 mx-auto max-w-4xl">
          {sportsList.map(({ id, icon: Icon, label }) => (
            <div
              key={id}
              onClick={() => handleSportSelect(id)}
              className={cn(
                'flex items-center justify-start gap-2 px-3 py-2 mx-2 cursor-pointer transition-all duration-200 ease-in-out min-w-fit',
                selectedSport === id
                  ? 'text-[#15222A]'
