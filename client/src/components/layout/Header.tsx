      ...
      <div className="container mx-auto flex items-center justify-between h-full px-20">
        <Link href="/" className="flex-shrink-0 flex items-center h-full bg-black">
          ...
          <span className="flex items-center gap-2 whitespace-nowrap">
            <span className="bg-gradient-to-br from-amber-300 via-yellow-500 to-orange-500 bg-clip-text text-transparent font-bold text-[clamp(2.219rem,2.6vw,2.642rem)] transition-all duration-200 hover:from-white hover:via-yellow-100 hover:to-amber-200 hover:drop-shadow-[0_0_8px_rgba(251,191,36,0.5)]" style={{fontFamily: 'Roboto Condensed, sans-serif', fontStretch: 'condensed', letterSpacing: '-0.05em'}}>CSSPORT</span>
          </span>
        </Link>
        ...
        <div className="flex items-center gap-[1.05rem]">
          {isAuthenticated && (
            <>
              <div className="font-semibold hover:text-amber-400 hover:bg-gray-800 hover:underline transition-colors duration-200 cursor-pointer px-2 py-1 rounded" style={{fontSize: '1.08rem'}}>{username ? username.charAt(0).toUpperCase() + username.slice(1) : ''}</div>
              <div 
