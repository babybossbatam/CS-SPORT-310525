...
const MyMainRight: React.FC<MyMainRightProps> = ({
  ...
  const filteredFixtures = useMemo(() => {
    ...
    console.log(
      `🔍 [MyMainRight] Processing ${fixtures.length} fixtures for date: ${selectedDate}`,
    );
    ...
    const filtered = fixtures.filter((fixture) => {
      ...
      if (fixture.fixture.date && fixture.fixture.status?.short) {
        ...
        const shouldInclude = (() => {
          ...
          if (selectedDate === todayString) {
            ...
            if (fixtureDateString < selectedDate) {
              console.log(
                `❌ [MyMainRight DATE FILTER] Excluding yesterday match: ${fixture.teams?.home?.name} vs ${fixture.teams?.away?.name} (${fixtureDateString} < ${selectedDate})`,
              );
              return false;
            }
        ...
        if (!shouldInclude) {
          console.log(
            `❌ [MyMainRight SMART FILTER] Match excluded: ${fixture.teams?.home?.name} vs ${fixture.teams?.away?.name}`,
            {
    ...
    console.log(
      `✅ [MyMainRight] After smart filtering: ${filtered.length} matches for ${selectedDate}`,
    );
    return filtered;
  ...
  const renderTabContent = () => {
    ...
    switch (activeTab) {
      case "match":
        return <MyMatchTabCard match={selectedFixture} />;
      case "lineups":
        return <MyLineUpTabCard match={selectedFixture} />;
      case "stats":
        return <MyMatchStatsTabCard match={selectedFixture} />;
      case "trends":
        return <MyTrendsTabCard match={selectedFixture} />;
      case "h2h":
        return <MyHeadtoHeadTabCard match={selectedFixture} />;
      default:
        return <MyMatchTabCard match={selectedFixture} />;
    }
  };
