...
const PlayerProfileModal: React.FC<PlayerProfileModalProps> = ({
  ...
  const getPlayerImageUrls = useCallback((playerId?: number): string[] => {
    if (!playerId) return [];
    return [
  ...
  const loadPlayerImage = useCallback(async () => {
    ...
    setImageError(true);
    setImageLoading(false);
  }, [playerId, playerImage, getPlayerImageUrls]);
  ...
  }, [isOpen, playerId, fetchPlayerStats, loadPlayerImage]);
