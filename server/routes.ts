...
// Get fixture by ID
  app.get("/api/fixtures/:id", async (req: Request, res: Response) => {
    try {
      ...
      console.error(`Error fetching fixture with ID ${req.params.id}:`, error);
      res.status(500).json({ message: "Failed to fetch fixture" });
    }
  });
  ...
  app.get("/api/fixtures/:id/events", async (req: Request, res: Response) => {
    try {
      ...
      console.log(`📊 [Routes] Fetching events for fixture: ${id}`);

      const events = await rapidApiService.getFixtureEvents(id);

      if (events === null) {
        return res.status(404).json({ message: 'Events not found for this fixture' });
      }

      console.log(`✅ [Routes] Returning ${events.length} events for fixture ${id}`);
      res.json(events);
    } catch (error) {
      console.error(`❌ [Routes] Error fetching events for fixture ${req.params.id}:`, error);
