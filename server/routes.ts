...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  apiRouter.get('/fixtures/analysis/comprehensive', async (req: Request, res: Response) => {
    try {
      console.log('🔍 Running comprehensive match analysis...');
      ...
      for (const date of dates) {
        try {
          console.log(`🌍 Analyzing fixtures for ${date}...`);
          const allFixtures = await rapidApiService.getFixturesByDate(date, true);

          totalMatches += allFixtures.length;
          allMatchesByDate[date] = allFixtures.length;

          // Collect statistics
          allFixtures.forEach(fixture => {
            if (fixture.league) {
              allLeagues.add(`${fixture.league.name} (ID: ${fixture.league.id})`);
              if (fixture.league.country) {
                ...
                countryStats[country].totalMatches++;
                countryStats[country].leagues.add(fixture.league.name);
                countryStats[country].matches.push({
          ...
          console.error(`❌ Error analyzing fixtures for ${date}:`, error);
        }
      }

      // Convert country stats to array and sort by match count
      const topCountries = Object.entries(countryStats)
        .map(([country, stats]: any) => ({
          country,
          totalMatches: stats.totalMatches,
          leagues: stats.leagues.size,
          leagueNames: Array.from(stats.leagues),
          sampleMatches: stats.matches.slice(0, 3)
        }))
        .sort((a, b) => b.totalMatches - a.totalMatches)
        .slice(0, 10);

      // Search for specific patterns
      const allFixtures = Object.values(allMatchesByDate).flat();
      const searchResults = {
        alAhlyInterMiami: false,
        uefaU21: false,
        fifaClubWorldCup: false,
        internationalFriendlies: false
      };

      // This would need to be implemented with actual fixture data analysis
      // For now, return mock search results

      const analysisResult = {
        totalMatches,
        uniqueLeagues: allLeagues.size,
        countries: allCountries.size,
        uniqueTeams: allTeams.size,
        matchesByDate: allMatchesByDate,
        topCountries,
        searchResults,
        generatedAt: new Date().toISOString()
      };

      console.log(`✅ Analysis complete: ${totalMatches} matches across ${allCountries.size} countries`);
