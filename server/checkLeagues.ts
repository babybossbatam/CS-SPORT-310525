  ...
  try {
    ...
    if (fifaClubWorldCup) {
      ...
      if (fifaClubWorldCup.seasons && fifaClubWorldCup.seasons.length > 0) {
        const currentSeason = fifaClubWorldCup.seasons.find(s => s.current) || fifaClubWorldCup.seasons[0];
      ...
      try {
        ...
        if (euroU21 && euroU21.league.name.toLowerCase().includes('u21')) {
          ...
          if (euroU21.seasons && euroU21.seasons.length > 0) {
            const currentSeason = euroU21.seasons.find(s => s.current) || euroU21.seasons[0];
            console.log(`✅ Current Season: ${currentSeason.year} (${currentSeason.start} to ${currentSeason.end})`);
          }
          foundEuroU21 = true;
          break;
    ...
    const allLeagues = await rapidApiService.getLeagues();
    const fifaLeagues = allLeagues.filter(league => 
