...
// Square league logo proxy endpoint  
router.get('/league-logo/square/:leagueId', async (req, res) => {
  const { leagueId } = req.params;
  
  try {
    console.log(`🔍 [Logo Proxy] Fetching square league logo for ID: ${leagueId}`);
    ...
    res.send(logoData);
    console.log(`✅ [Logo Proxy] Successfully proxied square league logo for ID: ${leagueId} (${logoData.length} bytes)`);
    
  } catch (error) {
    console.error(`❌ [Logo Proxy] Failed to fetch square league logo for ID: ${leagueId}:`, error?.message || 'Unknown error');
    
    // Fallback to regular league logo endpoint
    try {
      console.log(`🔄 [Logo Proxy] Redirecting to regular league logo for ${leagueId}`);
      res.redirect(`/api/league-logo/${leagueId}`);
    } catch (fallbackError) {
      console.error(`💥 [Logo Proxy] Fallback redirect failed for league ${leagueId}:`, fallbackError);
      res.status(404).send('Logo not found');
    }
  }
});
